...
var cartOpenedAt = 0;
    ...
    // MeshProtect Kit variants (S/M/L) — only these show a per-variant quantity badge in the drawer
    var MP_VARIANTS = ['52726522413237','52726522446005','52726522478773'];
    ...
    function updateCartUI(cart) {
      ...
      var html='';
      var totalSave=0, dispTotal=0;
      // MeshProtect bundle tier = total kits in cart -> per-kit price (checkout discount handled separately)
      var mpTotalQty=0;
      cart.items.forEach(function(it){ if(MP_VARIANTS.indexOf(String(it.variant_id))!==-1) mpTotalQty+=it.quantity; });
      var mpPerKit = mpTotalQty>=3?34.99:(mpTotalQty===2?39.99:49.99);
      ...
      items.forEach(function(item){
        ...
        var isMP=MP_VARIANTS.indexOf(vid)!==-1;
        var fin=item.final_line_price/100;
        var subbed=!!(item.selling_plan_allocation && item.selling_plan_allocation.selling_plan);
        var comp,save;
        if(isMP){ fin=mpPerKit*item.quantity; comp=49.99*item.quantity; save=Math.max(0,comp-fin); }
        else { var orig=(subbed?ORIG_SUB:ORIG_ONE)[vid]; if(orig!==undefined){comp=orig;save=Math.max(0,orig-fin);}else{save=Math.round(fin/3);comp=fin+save;} }
        totalSave+=save; dispTotal+=fin;
        ...
        html+='<div class="cart-item">'
          ...
          +'<div class="cart-item__info">'
            +'<div class="cart-item__head">'
              +'<div class="cart-item__main">'
                +'<p class="cart-item__title">'+(['52514698887349','52515952001205','52579196403893'].indexOf(vid)!==-1?'Pinemoor&reg; Tick<br>Protection Drops':(item.product_title||item.title))+'</p>'
                +'<div class="cart-item__left">'+variant+(MP_VARIANTS.indexOf(vid)!==-1?'<span class="cart-item__qty" style="display:inline-block;font-size:11.5px;font-weight:700;color:#085946;background:#eef3ef;border-radius:6px;padding:1px 8px;margin-top:4px;">Qty&nbsp;'+item.quantity+'</span>':'')+freeship+freegift+'</div>'
              +'</div>'
              +'<div class="cart-item__right">'
      ...
      var totalFinal=dispTotal;
      var totalComp=totalFinal+totalSave;
      var compEl=document.getElementById('cart-drawer-compare'); if(compEl){if(totalSave>0){compEl.textContent='$'+totalComp.toFixed(2);compEl.style.display='';}else{compEl.style.display='none';}}
      ...
      if(bundleEl&&bundleTextEl){var mainIt=cart.items[0];for(var mi=0;mi<cart.items.length;mi++){if(String(cart.items[mi].variant_id)!=='52748083724469'){mainIt=cart.items[mi];break;}}var bv=String(mainIt.variant_id); var bl=PM_BOTTLES[bv]||((mainIt.variant_title&&mainIt.variant_title!=='Default Title')?mainIt.variant_title:''); if(bl){bundleTextEl.textContent=bl.toUpperCase();bundleEl.style.display='';}else{bundleEl.style.display='none';}}
    }
